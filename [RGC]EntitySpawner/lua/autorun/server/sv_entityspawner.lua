...
net.Receive("sendEnt", function(len, ply)
	...
	entsOwners[ZKEntSpawn.ZK_Index] = ent:GetOwner() -- Log the Owner's name in a table.
...
if SERVER then
	hook.Add("PlayerDeath", "ZK_OnPlayerDeath", function(vic, inf, at)
		...
		if (table.HasValue(entsOwners, vic)) then -- If the player that died is an Owner of an Entity
